...
class Watson {
    ...
    List<Object> assist(String levelName, InvestigationLevel nextLevel, int drilldownLimit) {
        ...
        VerifiableTable actualResults =
                new KeyedVerifiableTableAdapter(queryResults[0], queryResults[0].getColumnCount() - 1);
        VerifiableTable expectedResults =
                new KeyedVerifiableTableAdapter(queryResults[1], queryResults[1].getColumnCount() - 1);
        ...
        if (!getLast(actualColumns).equals(getLast(expectedColumns))) {
            throw new IllegalArgumentException(String.format(
                    "Key columns must match at each investigation level [actual: %s, expected: %s]",
                    getLast(actualColumns), getLast(expectedColumns)));
        }
        ...
        if (Math.min(actualColumns.size(), expectedColumns.size()) > 1 && commonColumns.size() < 2) {
            throw new IllegalArgumentException(String.format(
                    "There must be at least 2 matching columns at each investigation level [actual: %s, expected: %s]",
                    getLast(actualColumns), getLast(expectedColumns)));
        }
        ...
        htmlFormatter.appendResults(
                levelName, Collections.singletonMap(levelDescription, results), Metadata.newEmpty());
    ...
    private List<Object> getRowKeys(ResultTable results, int drilldownLimit) {
        ...
        while (rowIndex < table.size() && rowKeys.size() < drilldownLimit) {
            ...
            int passedCount =
                    (int) values.stream().filter(ResultCell.IS_PASSED_CELL).count();
            int failedCount =
                    (int) values.stream().filter(ResultCell.IS_FAILED_CELL).count();
    ...
    private ResultTable findBreaks(String tableName, VerifiableTable actual, VerifiableTable expected) {
        ...
        Map<String, ResultTable> verifyTables =
                this.multiTableVerifier.verifyTables(expectedVerifiableTableResults, actualVerifiableTableResults);
