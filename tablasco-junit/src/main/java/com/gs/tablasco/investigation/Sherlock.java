...
public class Sherlock {
    ...
    public void handle(Investigation investigation, File outputFile) {
        ...
        while (!drilldownKeys.isEmpty() && (currentLevel = investigation.getNextLevel(drilldownKeys)) != null) {
            drilldownKeys = watson.assist(
                    "Investigation Level " + level + " (Top " + investigation.getRowKeyLimit() + ')',
                    currentLevel,
                    investigation.getRowKeyLimit());
            LOGGER.info("Got {} broken drilldown keys - {}", drilldownKeys.size(), outputFile);
            level++;
        }

        String message =
                "Some tests failed. Check test results file " + outputFile.getAbsolutePath() + " for more details.";
