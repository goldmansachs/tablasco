...
public class ListVerifiableTableTest
{
    ...
    public void testHeaderTypes() throws Exception
    {
        ...
        Assert.assertEquals("Test cast that used to be necessary for headers as List<String>", 2, new ListVerifiableTable(headersAsStrings, headersAndDataAsObjects).getRowCount());
    ...
    {
        VerifiableTable table = ListVerifiableTable.create(Arrays.asList("A", "B"), Collections.singletonList(Arrays.asList(1, 2)));
        Assert.assertEquals(2, table.getColumnCount());
        Assert.assertEquals(1, table.getRowCount());
        Assert.assertEquals("A", table.getColumnName(0));
        Assert.assertEquals("B", table.getColumnName(1));
    ...
    {
        VerifiableTable table = ListVerifiableTable.create(Arrays.asList("A", "B"), Collections.singleton(Arrays.asList(1, 2)));
        Assert.assertEquals(2, table.getColumnCount());
        Assert.assertEquals(1, table.getRowCount());
        Assert.assertEquals("A", table.getColumnName(0));
        Assert.assertEquals("B", table.getColumnName(1));
