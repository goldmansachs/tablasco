...
public class KeyColumnPartialMatcher implements PartialMatcher {
    ...
    public void match(
        ...
        for (UnmatchedIndexMap expected : allMissingRows) {
            ExpectedRowView expectedRowView = new ExpectedRowView(
                    this.expectedData, keyColumnIndices, this.columnComparators, expected.getExpectedIndex());
            missingByKey
                    .computeIfAbsent(expectedRowView, rowView -> new ArrayList<>(4))
                    .add(expected);
        }
        ...
        for (UnmatchedIndexMap actual : allSurplusRows) {
            ActualRowView actualRowView = new ActualRowView(
                    this.actualData, keyColumnIndices, this.columnComparators, actual.getActualIndex());
            surplusByKey
                    .computeIfAbsent(actualRowView, rowView -> new ArrayList<>(4))
                    .add(actual);
        }
